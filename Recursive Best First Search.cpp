...
public:
	...
	void enqueue(int n1, int n2, int n3, int n4)
	{
		queuenode *ptr, *temp;
		ptr = new queuenode;
		ptr->row = n1;
		...
		if (isempty() || ptr->current_total_cost < front->current_total_cost)
		{
		//	if (front != NULL)
		//		ptr->f_limit = front->current_total_cost;
		//	else
		//		ptr->f_limit = -1;
		...
		{
			temp = front;
			while (temp->next_q != NULL && temp->next_q->current_total_cost <= ptr->current_total_cost)
			{
				temp = temp->next_q;
			}
		//	if (temp->next_q == NULL)
			//ptr->f_limit = temp->next_q->current_total_cost;
	...
	string dequeue()
	{
		if (!isempty())
		{
			...
			int n1 = front->row;
			int n2 = front->col;
			int n3 = front->current_total_cost;
			temp = front->next_q;
			if (temp != NULL)
			{
				front->f_limit = temp->current_total_cost;
			}
			delete front;
			front = temp;
			no_of_items--;
...
void RecursiveBestFirst_Search(Node* head, Node* s, int size, edge* &e_head, int key_row, int key_col, int goal_row, int goal_col, string** arr)
{
	Priority_Queue q2(size);
	cout << "Recursive Best First Search:\n";
	cout << "============================\n";
	int count = 0;
	s = head;
	while (s != NULL)
	...
	if (key_row == s->row && key_col == s->col)
	{
		Node* u = head;
		s->cost = 0;
		q2.enqueue(s->row, s->col, s->heuristic_value, s->cost);
		u->cost = 0;
		while (!q2.isempty())
		{
			string x = q2.dequeue();
			...
			while (v != NULL)
			{
				...
				{
					v->color = "grey";
					q2.enqueue(v->edge_row, v->edge_col, v->heuristicValue_e, v->cost_e);
...
int main()
{
	...
	RecursiveBestFirst_Search(head, s, size, temp2->e_head, start_key_row, start_key_col, goal_key_row, goal_key_col, arr);
